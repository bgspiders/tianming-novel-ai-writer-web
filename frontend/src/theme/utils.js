...
export function mixColors(base, target, weight) {
...
export function shiftHue(color, degrees, saturationDelta = 0, lightnessDelta = 0) {
...
export function applyTokensToDocument(tokens, dark, themeId) {
...
export function calculateSunTimes(date, latitude, longitude) {
    try {
        ...
        const declination = (Math.asin(Math.sin((obliquity * Math.PI) / 180) * Math.sin((trueLongitude * Math.PI) / 180)) *
            180) /
            Math.PI;
        const acosInput = (Math.cos((90.833 * Math.PI) / 180) -
        ...
        if (sunrise < 0 || sunrise > 12)
            sunrise = 6;
        if (sunset < 12 || sunset > 24)
            sunset = 18;
        return {
...
export function getUpcomingHolidays(fromDate, limit = 6) {
