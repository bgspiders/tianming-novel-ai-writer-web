import type { Locale, TranslationDictionary } from './core'
...
export const messages: TranslationDictionary = {
  'zh-CN': {
    ...
    home: {
      ...
      summary:
        '按原生天命 2.8.7 的推荐路线组织入口：先配置模型和拆书素材，再维护世界观、角色、势力、位置、剧情、大纲、分卷、章节和蓝图；修改设计或规划后必须重新打包，再进入创作台生成与校验。',
      quickActions: {
        aiModels: '模型配置',
        bookAnalyses: '智能拆书',
        creativeMaterials: '创意素材',
    ...
    health: {
      title: '健康检查',
      hint:
        '调用 GET /api/health 确认后端已在线，并返回版本号与时间戳信息。如果请求失败，请确认 API 正在 http://localhost:38721 运行。',
    ...
    aiTest: {
      title: 'AI 流式测试',
      hint:
        '此页面会向 POST /api/ai/test-completion 发起请求，通过 SignalR ChatHub 接收流式 token，并在浏览器中实时渲染结果。',
  ...
  en: {
    ...
    home: {
      ...
      summary:
        'Entrypoints now follow the native TM 2.8.7 route: configure models and source analysis, maintain design rules and writing plans, repackage changed data, then generate and validate in the writing workspace.',
      quickActions: {
    ...
    health: {
      title: 'Health Check',
      hint:
        'Call GET /api/health to confirm the backend is online and returning version plus timestamp data. If the request fails, verify the API is running on http://localhost:38721.',
    ...
    aiTest: {
      ...
      hint:
        'This page sends a request to POST /api/ai/test-completion, receives streamed tokens through SignalR ChatHub, and renders the result live in the browser.',
