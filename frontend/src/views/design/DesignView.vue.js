...
async function rewriteChapterPlanSummaries() {
    try {
        await ElMessageBox.confirm('将按当前项目、源书、分类和筛选条件批量重写章节计划简介。标题、章节号和实体引用不会修改。是否继续？', t('layout.dialogs.confirm'), { type: 'warning' });
    }
    catch {
        return;
    }
    ...
    try {
        ...
        ElMessage.error(err.message ?? '批量重写章节计划简介失败');
    }
    finally {
