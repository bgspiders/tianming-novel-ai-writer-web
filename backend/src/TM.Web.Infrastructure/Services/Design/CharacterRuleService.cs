...
public class CharacterRuleService : ICharacterRuleService
{
    ...
    public async Task<IReadOnlyList<CharacterRuleDto>> ListAsync(DesignListQuery query, CancellationToken ct = default)
    {
        ...
        var rows = await ApplyDefaultOrder(_db.CharacterRules.AsQueryable().ApplyFilter(query)).ToListAsync(ct);
    ...
    private static IOrderedQueryable<CharacterRule> ApplyDefaultOrder(IQueryable<CharacterRule> query)
